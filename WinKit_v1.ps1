...
Write-Host "Unblocking all files in the current directory..."
Set-Location $PSScriptRoot
Get-ChildItem -Path $PSScriptRoot -Recurse | Unblock-File
write-host Files In current directory Un-Blocked
pause
# Allow double-click to run PowerShell scripts
# PowerShell script to add a registry entry similar to the batch command

# Set the default command for PowerShell files
Set-ItemProperty -Path "Registry::HKEY_CLASSES_ROOT\Applications\powershell.exe\shell\open\command" -Name "(Default)" -Value "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -NoLogo -ExecutionPolicy unrestricted -File `"%1`"" -Type String -Force

Set-ItemProperty -Path "Registry::HKEY_CLASSES_ROOT\Applications\powershell.exe\shell\open\command" -Name "(Default)" -Value "C:\Program Files\PowerShell\7\pwsh.exe -NoLogo -ExecutionPolicy unrestricted -File `"%1`"" -Type String -Force


write-host Allow double-click to run PowerShell scripts.
pause
# Allow PowerShell scripts
$regPath1 = "HKCU:\SOFTWARE\Microsoft\PowerShell\1\ShellIds\Microsoft.PowerShell"
$regPath2 = "HKLM:\SOFTWARE\Microsoft\PowerShell\1\ShellIds\Microsoft.PowerShell"
New-ItemProperty -Path $regPath1 -Name "ExecutionPolicy" -Value "Unrestricted" -PropertyType String -Force | Out-Null
New-ItemProperty -Path $regPath2 -Name "ExecutionPolicy" -Value "Unrestricted" -PropertyType String -Force | Out-Null
write-host Allow PowerShell scripts
# Close the current PowerShell window
Write-Output Enabled Powershell Scripts + Unblocked Files
pause
